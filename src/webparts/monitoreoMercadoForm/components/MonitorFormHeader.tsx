...
const MonitorFormHeader: React.FC<IMonitorFormHeaderProps> = (props) => {
    ...
    const listaClientesFiltro: Cliente[] = listaClientes.filter((item: Cliente) => item.Unidad?.Id === unidad?.Id && Number(item.Anho) === new Date().getFullYear() - 1 && item.CNG?.Correo === user.email);

    console.log(listaClientes.filter((item: Cliente) => item.CNG?.Correo === user.email))
